...
'''
def linear_svm(features2, target2, num_steps2, learning_rate2, _lambda, add_intercept = False):
    if add_intercept:
...
def SVM(train_set, test_set):
    ...
    weights2 = linear_svm(train_features2, train_labels2, num_steps2 = 500, learning_rate2 = 0.7, _lambda = 0.07, add_intercept=False)
...
def cost_func(features, target, weights, _lambda):
    scores = np.dot(features, weights)
    cf = np.sum( target*scores - np.log(1 + np.exp(scores)) )  +  _lambda/(2) * sum(weights**2)
    return cf

def logistic_regression(features, target, num_steps, learning_rate, _lambda, add_intercept = False):
    if add_intercept:
        #print "features before",features.shape
    ...
    for step in xrange(num_steps):
        ...
        #if step % 10 == 0:
            #print cost_func(features, target, weights, _lambda)
...
def LR (train_set, test_set):
    ...
    #learning part
    #step_size = learning rate
    weights = logistic_regression(train_features, train_labels, num_steps = 5000, learning_rate = 0.05, _lambda = 0.1, add_intercept=True)
