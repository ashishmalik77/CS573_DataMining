...
for i in range (0,len(t_frac)):
    ...
    for j in range (1,11):
        ...
        predictions_train_cv_4, predictions_test_cv_4 = decision_tree(train_set.values.tolist(), test_set.values.tolist(), 8, 50)
        label_train_4 = train_set.values[0:, 49:50]
        label_test_4 = test_set.values[0:, 49:50]

        train_accuracy_DT_4 = calculate_accuracy(predictions_train_cv_4, label_train_4)
        accuracy_train_DT_arr.append(train_accuracy_DT_4)
        test_accuracy_DT_4 = calculate_accuracy(predictions_test_cv_4, label_test_4)
        accuracy_test_DT_arr.append(test_accuracy_DT_4)
...
for i in range (0,len(t_frac)):
    ...
    for j in range (1,11):
        train_set, test_set = union_trainset(j, t_frac[i], S1, S2 , S3 , S4 , S5 , S6 , S7 , S8 , S9 , S10)
        predictions_train_cv_4, predictions_test_cv_4 = bagging(train_set.values.tolist(), test_set.values.tolist(), 8, 50, n_trees)
        label_train_4 = train_set.values[0:, 49:50]
        label_test_4 = test_set.values[0:, 49:50]
    ...
    #validation_BT.append(accuracy_test_BT_arr)
...
for i in range (0,len(t_frac)):
    ...
    for j in range (1,11):
        train_set, test_set = union_trainset(j, t_frac[i], S1, S2 , S3 , S4 , S5 , S6 , S7 , S8 , S9 , S10)
        predictions_train_cv_4, predictions_test_cv_4 = random_forest(train_set.values.tolist(), test_set.values.tolist(), 8, 50, n_trees, n_features)
        label_train_4 = train_set.values[0:, 49:50]
        label_test_4 = test_set.values[0:, 49:50]
    ...
    #validation_RF.append(accuracy_test_RF_arr)
...
'''
########### Hypothesis testing DT vs BT ###########

p_value1 = stats.ttest_rel (avg_accuracy_test_DT,avg_accuracy_test_BT)
print "p value DT and BT", p_value1
'''

########### Hypothesis testing BT vs RF ###########
#for i in range (0,len(t_frac)):
    #for j in range (0, len(validation_RF)):
#print len(validation_BT), len(validation_RF)
...
p_value5 = stats.ttest_rel (validation_BT[40:50],validation_RF[40:50])
print "p value BT and RF for frac 0.05 : ", p_value1
print "p value BT and RF for frac 0.075 : ", p_value2
print "p value BT and RF for frac 0.1 : ", p_value3
print "p value BT and RF for frac 0.15 : ", p_value4
print "p value BT and RF for frac 0.2 : ", p_value5

'''
########### Hypothesis testing DT vs RF ###########
p_value3 = stats.ttest_rel (avg_accuracy_test_DT, avg_accuracy_test_RF)
print "p value DT and RF", p_value3
'''
plt.errorbar(t_frac, avg_accuracy_test_DT, yerr=sterr_test_DT, marker='o', color='blue',ecolor='gray',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy DT')
plt.errorbar(t_frac, avg_accuracy_test_BT, yerr=sterr_test_BT, marker='o', color='green',ecolor='purple',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy BT')
plt.errorbar(t_frac, avg_accuracy_test_RF, yerr=sterr_test_RF, marker='o', color='red',ecolor='black',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy RF')
plt.xlabel('Training fraction')
plt.ylabel('Model accuracy')
plt.title('Learning curve')
#plt.xticks(index, ('0.01', '0.1', '0.2', '0.5', '0.6', '0.75', '0.9', '1'))
plt.xlim((.01,0.3))                 #Set X-axis limits
