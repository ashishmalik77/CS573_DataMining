...
WC_SSD_std_ds1 = []
SC_std_ds1 = []
for m in range (0,len(K)): #different values of K
	WC_SSD_r = []
	SC_r = []
	for r in range (0,10):
		#np.random.seed(r)
		WC_SSD, SC = kmeans(X,label,N,num_iter,K[m])
		WC_SSD_r.append(np.mean(WC_SSD))
		SC_r.append(np.mean(SC))
	WC_SSD_ds1.append(np.mean(WC_SSD_r))
	SC_ds1.append(np.mean(SC_r))	
	WC_SSD_std_ds1.append(np.std(WC_SSD_r))
	SC_std_ds1.append(np.std(SC_r))


######.....error bar ..........########
fig, ax = plt.subplots()
plt.errorbar(K, WC_SSD_ds1, yerr=WC_SSD_std_ds1, marker='o', color='blue',ecolor='red',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of WC-SSD for dataset1')
#plt.errorbar(K, SC_ds1, yerr=SC_std_ds1, marker='^', color='green',ecolor='purple',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of SC for dataset1')
#plt.errorbar(K, avg_accuracy_test_RF, yerr=sterr_test_RF, marker='o', color='red',ecolor='black',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy RF')
plt.xlabel('K')
plt.ylabel('WC-SSD')
plt.title('k-means sensitivity')
#plt.xticks(index, ('0.01', '0.1', '0.2', '0.5', '0.6', '0.75', '0.9', '1'))
plt.xlim((1,35))            #Set X-axis limits here K = [2, 4, 8, 16, 32]
...
for m in range (0,len(K)): #different values of K
	...
	for r in range (0,10):
		...
		WC_SSD_r.append(np.mean(WC_SSD))
		SC_r.append(np.mean(SC))
	WC_SSD_ds2.append(np.mean(WC_SSD_r))
	SC_ds2.append(np.mean(SC_r))	
	WC_SSD_std_ds2.append(np.std(WC_SSD_r))
	SC_std_ds2.append(np.std(SC_r))


######.....error bar ..........########
fig, ax = plt.subplots()
plt.errorbar(K, WC_SSD_ds2, yerr=WC_SSD_std_ds2, marker='o', color='blue',ecolor='red',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of WC-SSD for dataset2')
#plt.errorbar(K, SC_ds2, yerr=SC_std_ds2, marker='^', color='green',ecolor='purple',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of SC for dataset2')
#plt.errorbar(K, avg_accuracy_test_RF, yerr=sterr_test_RF, marker='o', color='red',ecolor='black',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy RF')
plt.xlabel('K')
plt.ylabel('WC-SSD')
plt.title('k-means sensitivity')
...
for m in range (0,len(K)): #different values of K
	...
	for r in range (0,10):
		...
		WC_SSD_r.append(np.mean(WC_SSD))
		SC_r.append(np.mean(SC))
	WC_SSD_ds3.append(np.mean(WC_SSD_r))
	SC_ds3.append(np.mean(SC_r))	
	WC_SSD_std_ds3.append(np.std(WC_SSD_r))
	SC_std_ds3.append(np.std(SC_r))


######.....error bar ..........########
fig, ax = plt.subplots()
plt.errorbar(K, WC_SSD_ds3, yerr=WC_SSD_std_ds3, marker='o', color='blue',ecolor='red',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of WC-SSD for dataset3')
#plt.errorbar(K, SC_ds3, yerr=SC_std_ds3, marker='^', color='green',ecolor='purple',elinewidth=1, capsize=2, barsabove = True, label='Mean and std of SC for dataset3')
#plt.errorbar(K, avg_accuracy_test_RF, yerr=sterr_test_RF, marker='o', color='red',ecolor='black',elinewidth=1, capsize=2, barsabove = True, label='Validation Accuracy RF')
plt.xlabel('K')
plt.ylabel('WC-SSD')
plt.title('k-means sensitivity')
